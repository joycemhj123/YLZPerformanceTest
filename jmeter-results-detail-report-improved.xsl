				...
				function method2(tableid) 
{
    ...
    {    
 	 k++;
       for(j=0;j<5;j++)
	  {
   	         oSheet.Cells(k, j + 1).value = curTbl.rows(i).cells(j).innerText;
          }
    }
    oXL.Visible = true;
	...
	<input type="button" onclick="javascript:method2('reports');" value="click"/>
		...
		<tr valign="top">
			...
			<xsl:variable name="allMinTime">
				<xsl:call-template name="min">
					<xsl:with-param name="nodes" select="/testResults/*" />
				</xsl:call-template>
			</xsl:variable>
			...
			<xsl:variable name="throughPut" select="60000 div $allAverageTime" />
...
<xsl:template name="pagelist">
	<h2>Pages</h2>
	<table class="details" border="0" cellpadding="5" cellspacing="2" width="95%" id="reports">
		<tr valign="top">
			<th>URL</th>
			<th>Failures</th>
			<th>Success Rate</th>
		...
		<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]">
			...
			<xsl:variable name="minTime">
				<xsl:call-template name="min">
					<xsl:with-param name="nodes" select="../*[@lb = current()/@lb]" />
				</xsl:call-template>
			</xsl:variable>
			...
			<xsl:variable name="throughPut" select="60000 div $averageTime" />
			<tr valign="top">
				<xsl:attribute name="class">
					<xsl:choose>
            ...
            <xsl:if test="position()=floor(number($count *0.9)+1)">
...
<xsl:template name="display-throughput">
	<xsl:param name="value" />
	<xsl:value-of select="format-number($value,'0.0 /min')" />
</xsl:template>
